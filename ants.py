...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location=self.place
        range_counter = 1 
        if location.bees and (self.min_range <= range_counter <= self.max_range):
            return random_or_none(location.bees)
        else: 
            while location.entrance != hive:
                if location.entrance.bees:
                    if self.min_range <= range_counter <= self.max_range:
                        return random_or_none(location.entrance.bees)
    ...
    def action(self, colony):
        ...
        if(self.digesting):
            ...
            if(self.place.bees):
                ...
                selected_bee = random_or_none(self.place.bees)
                self.eat_bee(selected_bee)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.real_queen:
            ...
            location = self.place.exit #first location behind the placed queen ant 
            while location != None: #check to see if there is no exit 
                if location.ant != None: #Check to see if there is an ant                   
                    if isinstance(location.ant, BodyguardAnt):
                        if location.ant.ant and location.ant.ant not in self.ants_doubled:
                            self.ants_doubled += [location.ant.ant]
                            location.ant.ant.damage *= 2
                    if location.ant not in self.ants_doubled:
                        self.ants_doubled += [location.ant]
